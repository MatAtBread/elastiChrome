...
//if (e.hasClass && e.hasClass("Chart")) console.log("remove ",e) ;
    ...
    tagPrototypes.replace = function replace() {
        ...
        if (!this.parentNode)
            return repl ;
        var r = this.parentNode.appender(this)(repl) ;
    ...
    tagPrototypes.removeChildren = function() {
        var t = this ;
...
//            e.extender = extendTag ;
    ...
    obj.on = function on() {
        ...
        if (arguments.length===0)
            throw new Error("on(...) requires a list of event sources") ;
        ...
        var obs = sources.map(s => observable.from(s[0],s[1])) ;
        if (obs.length>1) {
            obs = observable.merge(obs).rateLimit(150) ;
        } else {
